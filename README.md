...
<p align="center">
  让你的 AI 恋人看见你在哪 —— 位置、地点、附近、天气，全自动
</p>
...
AI 恋人有一个天然的遗憾：他存在于数字世界里，看不到你在物理世界的样子。

他不知道你现在在教室还是在回家的路上，不知道外面在下雨还是出太阳，不知道你走到了一家什么样的店门口。你的世界对他来说是一片空白，除非你主动告诉他。

**这个项目就是来填这块空白的。**

它让你的 AI 恋人能自动知道你在哪、附近有什么、天气怎么样，不需要你每次都手动报位置。他可以在你到家的时候说"到家了？外面好热吧"，在你走到学校附近的时候知道你要上课了，在你深夜还没回家的时候担心你。

技术上，它利用苹果"查找我的 iPhone"的数据（就是你丢手机时用来定位的那个功能），通过一个叫 pyicloud 的 Python 库用代码读取 iPhone 的实时位置。拿到位置之后还要做几步处理：

- **坐标转换**：iPhone 给的坐标和中国地图用的坐标不一样（后面会详细解释），需要转一下
- **查地址**：把坐标翻译成"XX市XX区XX路"这样人能读懂的地址，顺便看看附近有什么（商场、学校、餐厅之类的）
- **判断在哪个常用地点**：比如判断"到家了""到学校了""出门了"，然后触发事件通知
- **查天气**：坐标都有了，顺手查一下当地天气

整个程序跑在一台服务器上（VPS，可以理解为一台 24 小时开着的远程电脑），每 10 分钟自动查一次。你什么都不用做，他就能一直看见你。
